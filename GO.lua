    ...
    for i,v in pairs(game:GetService("Workspace").Vehicles:GetChildren()) do
        if v:FindFirstChild("Seat") then
            if v.Seat.PlayerName.Value == game:GetService("Players").LocalPlayer.Name then
                local StartTween =  game:GetService("TweenService"):Create(v.PrimaryPart, TweenInfo.new(2, Enum.EasingStyle.Linear), {CFrame = CFrame.new(v.PrimaryPart.Position.X, 500, v.PrimaryPart.Position.Z)})
                StartTween:Play()
                StartTween.Completed:Wait()
                
                local tween = game:GetService("TweenService"):Create(v.PrimaryPart, TweenInfo.new(Time, Enum.EasingStyle.Linear), {CFrame = CFrame.new(CFrameEnd.X, 500, CFrameEnd.Z)})
                tween:Play()
                tween.Completed:Wait()
                
                local FinalTween = game:GetService("TweenService"):Create(v.PrimaryPart, TweenInfo.new(2, Enum.EasingStyle.Linear), {CFrame = CFrame.new(CFrameEnd.X, CFrameEnd.Y, CFrameEnd.Z)})
                FinalTween:Play()
